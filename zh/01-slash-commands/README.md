...
| `/doctor` | 檢查安裝健康狀態 |
| `/effort [low|medium|high|xhigh|max|auto]` | 設定推理強度；`xhigh` 與 `max` 需要 Opus 4.7（v2.1.111） |
| `/exit` | 退出 REPL（別名：`/quit`） |
| `/export [filename]` | 將當前對話匯出為檔案或剪貼簿內容 |
| `/extra-usage` | 配置額外用量以應對速率限制 |
| `/fast [on|off]` | 切換快速模式 |
| `/feedback` | 提交反饋（別名：`/bug`） |
| `/focus` | 進入 focus view，隱藏 UI 元素以專注工作（v2.1.110） |
| `/help` | 顯示幫助 |
| `/hooks` | 檢視 hook 配置 |
| `/ide` | 管理 IDE 整合 |
...
| `/keybindings` | 開啟快捷鍵配置 |
| `/less-permission-prompts` | 啟動「減少權限提示」skill,自動授權常用操作（v2.1.111） |
...
| `/theme` | 更改顏色主題,可從 `~/.claude/themes/` 載入自訂 JSON 主題（v2.1.118） |
| `/tui` | 無閃爍全螢幕渲染（v2.1.110） |
| `/ultrareview` | 雲端全面 code review skill（v2.1.111） |
| `/usage` | 顯示 API 使用統計,合併原 `/cost` + `/stats`（v2.1.118） |
| `/voice` | 切換按住說話語音輸入 |
...
- 新增 `/effort`，其中 `max` 與新的 `xhigh` 級別需要 Opus 4.7（v2.1.111）；可在 REPL 中用左右箭頭調整，或用 `/effort` 開啟互動式滑塊
- 新增 `/tui`、`/focus`（v2.1.110），分別提供無閃爍全螢幕渲染與 focus view
- 新增 `/ultrareview`、`/less-permission-prompts` skills（v2.1.111）
- `/cost` 與 `/stats` 合併為單一 `/usage`（v2.1.118）；舊指令仍保留但建議改用 `/usage`
- 新增 `/voice`，用於按住說話語音輸入
