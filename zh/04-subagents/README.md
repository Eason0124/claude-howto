...
設定環境變數 `CLAUDE_CODE_FORK_SUBAGENT=1` 可啟用 **forked subagents**:每個 subagent 在獨立程序中執行,擁有完全獨立的上下文與權限模型,避免與主會話互相汙染。

```bash
export CLAUDE_CODE_FORK_SUBAGENT=1
claude "implement feature X with parallel reviewers"
```
...
| 模式 | 上下文隔離 | 程序隔離 | 並行 |
|------|-----------|---------|------|
| 預設 subagent | ✅ 邏輯隔離 | ❌ 共用程序 | 受 task 排程 |
| Forked subagent | ✅ 邏輯隔離 | ✅ 獨立程序 | 真正並行 |

### 適用場景

- 多個審查者同時掃同一份程式碼,確保不互相影響判斷
- 並行跑長任務(測試、安全掃描、效能分析),互不阻塞
- 高敏感任務需要嚴格上下文邊界

### 注意事項

- 開啟後資源消耗較高(每個 fork 各佔記憶體)
- subagent 之間不共享記憶,僅最終結果回傳主會話
